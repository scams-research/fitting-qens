...
bounds_2s = np.array([[0, 10], [0, 1], [0, 10], [0, 1]])

def two_stretch(t, *params):
    t1, beta1, t2, beta2 = params
    stretched_exp1 = np.exp(-(t / t1) ** beta1)
    stretched_exp2 = np.exp(-(t / t2) ** beta2)
    return np.array([stretched_exp1, stretched_exp2])

bounds_list = [bounds_2s] 
names = ['lit_approach']
perpendicular_models = [two_stretch]
param_names = {'lit_approach': ['t1', 'beta1', 't2', 'beta2']}
