...
from bounds import D_fick, D_rot, normaliser, u2, bkg
...
def high_level_model(omega: np.ndarray, q: float, *params: List[float]):
    D_fick, D_rotational1, r1, normaliser, u2 = params
    # fickian
    amp_t = np.exp(-(q**2) * u2 / 3)
    gamma_fick = fickian(q, D_fick)
    lorentzian_fick = lorentzian(omega, gamma_fick, amp_t)

    # Rotation
    amp1 = spherical_jn(1, q * r1) ** 2
    amp2 = spherical_jn(2, q * r1) ** 2
    gamma_rotational1 = rotational_diffusion(q, D_rotational1)
...
def fixed_fick_rotate(omega: np.ndarray, q: float, *params: List[float]):
    D_fick, D_rotational1, normaliser, u2, bkg = params

    r1 = 2.48
    HL = high_level_model(omega, q, D_fick, D_rotational1, r1, normaliser, u2)
...
res360_b["energy"] = 3.60
res197_b["energy"] = 1.97


bounds = [D_fick, D_rot] + [u2] + [normaliser] + q_bounds + [normaliser] + q_bounds197
