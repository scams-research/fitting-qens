...
def fixed_fick_rotate_sample(resolution: PletData, q, *params) -> np.ndarray:
    ...
    omega_masked = resolution.omega_mid.values[
        np.invert(resolution.data.masks["omega"].values)
    ]
    ...
    model_unconvolved = fixed_fick_rotate(
        omega_conv, q_mid[q], *global_params, q_params[0], q_params[q + 1]
    )
...
with open(str(paths.data) + "/pLET_aniso_model.pkl", "rb") as f:
